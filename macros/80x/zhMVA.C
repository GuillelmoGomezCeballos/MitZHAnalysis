...
void zhMVA(unsigned int mode, string mediator="", string signal_model="", string extra_string="") {
  ...
  for(unsigned int nModel=0; nModel<nSigModels; nModel++) {
    ...
    factory->AddTree(signal_mva_tree_[nModel], "Signal");
  }
  // Background trees
  TTree *Zjets_mva_tree, *EM_mva_tree, *WZ_mva_tree, *ZZ_mva_tree, *VVV_mva_tree;
  if(mode==4) {
    Zjets_mva_tree    = (TTree*) mva_input_trees->Get("bkg_mva_tree_Zjets"); factory->AddTree(Zjets_mva_tree , "Z+jets");
    EM_mva_tree       = (TTree*) mva_input_trees->Get("bkg_mva_tree_EM");    factory->AddTree(EM_mva_tree    , "Non-prompt");
    VVV_mva_tree      = (TTree*) mva_input_trees->Get("bkg_mva_tree_VVV");   factory->AddTree(VVV_mva_tree   , "Non-prompt");
    WZ_mva_tree       = (TTree*) mva_input_trees->Get("bkg_mva_tree_WZ");    factory->AddTree(WZ_mva_tree    , "WZ");
    ZZ_mva_tree       = (TTree*) mva_input_trees->Get("bkg_mva_tree_ZZ");    factory->AddTree(ZZ_mva_tree    , "ZZ");
    factory->SetWeightExpression( "mva_weight", "Signal");    
    factory->SetWeightExpression( "mva_weight", "Z+jets");    
    factory->SetWeightExpression( "mva_weight", "Non-prompt");
    factory->SetWeightExpression( "mva_weight", "WZ");        
    factory->SetWeightExpression( "mva_weight", "ZZ");        
  } else {
    EM_mva_tree       = (TTree*) mva_input_trees->Get("bkg_mva_tree_EM");    factory->AddBackgroundTree(EM_mva_tree    , 1);
    WZ_mva_tree       = (TTree*) mva_input_trees->Get("bkg_mva_tree_WZ");    factory->AddBackgroundTree(WZ_mva_tree    , 1);
    ZZ_mva_tree       = (TTree*) mva_input_trees->Get("bkg_mva_tree_ZZ");    factory->AddBackgroundTree(ZZ_mva_tree    , 1);
  ...
  if(mode==1) {
    ...
    factory->AddVariable( "mva_ptl1mptl2_over_ptll"           , "Lepton Balance"                    , ""    , 'F');
    TCut preselectionCut = "mva_MET>120 && mva_3lveto==1 && mva_mll_minus_mZ <= 30 && mva_ntaus==0 && mva_btag_veto==1 && (mva_delphi_jet_MET > 0.5 || mva_delphi_jet_MET < 0)";
    factory->PrepareTrainingAndTestTree(preselectionCut, "");
    ...
    factory->BookMethod( TMVA::Types::kBDT, "BDT_massDependent_"+signal_model, "!H:!V:NTrees=1000:MinNodeSize=2.5%:MaxDepth=4:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=10000:PruneMethod=NoPruning");
  } else if(mode==4) {
    factory->AddVariable( "mva_balance"                       , "Balance"                           , ""    , 'F');
    factory->AddVariable( "mva_cos_theta_star_l1"             , "cos #theta^{*}_{l1}"               , ""    , 'F');
    factory->AddVariable( "TMath::Abs(mva_cos_theta_CS_l1)"   , "| cos #theta^{CS}_{l1} |"          , ""    , 'F');
    ...
    factory->AddVariable( "mva_ptl2"                          , "p_{T}^{l2}"                        , "GeV" , 'F');
    factory->AddVariable( "mva_ptl1mptl2_over_ptll"           , "Lepton Balance"                    , ""    , 'F');
    TCut preselectionCut = "mva_MET>130 && mva_3lveto==1 && mva_mll_minus_mZ <= 30 && mva_ntaus==0 && mva_btag_veto==1 && (mva_delphi_jet_MET > 0.5 || mva_delphi_jet_MET < 0)";
    factory->PrepareTrainingAndTestTree(preselectionCut, "");
    factory->BookMethod( TMVA::Types::kBDT, "BDT_multiClass_"+signal_model+(extra_string == "" ? "" : "_"+extra_string), "!H:!V:NTrees=400:MinNodeSize=5%:MaxDepth=6:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.50:nCuts=10000:PruneMethod=NoPruning");
  }
