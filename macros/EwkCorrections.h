...
std::vector<std::vector<float>> readFile_and_loadEwkTable(TString dtag){
  ...
  TString path = cmssw_path+"/src/MitZHAnalysis/data/";

  if(dtag.Contains("ZZ")) name = path+"ZZ_EwkCorrections.dat";
  if(dtag.Contains("WZ")) name = path+"WZ_EwkCorrections.dat";
...
double getEwkCorrections(TString dtag, const std::vector<std::vector<float>> & Table, double & ewkCorrections_error){
  double kFactor = 1.;
  ...
  if(isWZ){
  	  ...
  	  if(genWBosons[0].pdgId() > 0) gamma_induced_uncertainty = 0.;
  	  if(genWBosons[0].pdgId() < 0) gamma_induced_uncertainty = 0.;
  	  ewkCorrections_error = sqrt(pow(ewkCorrections_error,2) + pow(gamma_induced_uncertainty,2));
  }

  return kFactor; //ewkCorrections_error is updated in the code too...
}
